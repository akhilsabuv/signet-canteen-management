...
def edit_canteen_timings():
    ...
    if request.method == 'POST':
        try:
            ...
            # Get all canteen names from the form
            canteen_names = request.form.getlist('canteen_name[]')
            start_times = request.form.getlist('start_time[]')
            end_times = request.form.getlist('end_time[]')
            descriptions = request.form.getlist('description[]')

            print(f"Processing {len(canteen_names)} canteen timings")

            # Get existing timings for comparison
            ...
            print("Existing timings:", existing_timings)

            # Track processed timings to identify deleted ones
            ...
            for i in range(len(canteen_names)):
                ...
                print(f"\nProcessing timing {i+1}: {canteen_name} ({start_time} - {end_time})")
                ...
                # Process shifts for this timing
                shift_keys = [k for k in request.form.keys() if k.startswith(f'shifts[{i}]')]
                shifts = [request.form[k] for k in shift_keys]
                print(f"Processing shifts for timing {timing_id}: {shifts}")

                # Insert new shift associations
                for shift in shifts:
                    cursor.execute("""
                        INSERT INTO canteen_timing_shifts (timing_id, shift)
                        VALUES (?, ?)
                    """, (timing_id, shift))
                    print(f"Added shift {shift} to timing {timing_id}")
            ...
            if timing_ids_to_delete:
                print(f"Removing deleted timings: {timing_ids_to_delete}")
        ...
        try:
            ...
            for timing in timings:
                cursor.execute("""
                    ...
                    WHERE timing_id = ?
                """, (timing.id,))
                timing.shifts = [row.shift for row in cursor.fetchall()]

            # Get allowed shifts for the form
            cursor.execute("SELECT DISTINCT shift FROM shifts ORDER BY shift")
            allowed_shifts = [row.shift for row in cursor.fetchall()]

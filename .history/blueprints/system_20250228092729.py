...
def live_data():
    ...
    try:
        ...
        # Get current active canteen timing
        cursor.execute("""
            SELECT 
                canteen_name,
                CONVERT(varchar, start_time, 108) as start_time,
        ...
        if current_timing:
            current_shift = f"{current_timing.canteen_name} ({current_timing.start_time} - {current_timing.end_time})"
        else:
            current_shift = "No Active Canteen"

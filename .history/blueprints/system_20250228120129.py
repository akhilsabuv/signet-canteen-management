        ...
        cursor.execute("""
            ...
            BEGIN
                CREATE TABLE canteen_timing_shifts (
                    ...
                    shift_name VARCHAR(50) NOT NULL,
                    FOREIGN KEY (timing_id) REFERENCES canteen_timings(id) ON DELETE CASCADE
                )
            END
        """)
        conn.commit()
        ...
        if request.method == 'POST':
            try:
                ...
                for i in range(len(canteen_names)):
                    ...
                    try:
                        if canteen_name in existing_timings:
                            ...
                            print(f"Cleared shifts for timing ID: {timing_id}")
                        ...
                        if selected_shifts:
                            for shift in selected_shifts:
                                cursor.execute("""
                                    ...
                                    VALUES (?, ?)
                                """, (timing_id, shift))
                                print(f"Inserted shift {shift} for timing ID: {timing_id}")
                ...
                for timing_id in timing_ids_to_delete:
                    ...
                    print(f"Deleted timing ID: {timing_id}")

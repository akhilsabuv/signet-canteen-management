...
def edit_canteen_timings():
    ...
    try:
        if request.method == 'POST':
            ...
            print("\nCanteen Names:", canteen_names)
            print("Start Times:", start_times)
            print("End Times:", end_times)
            print("Descriptions:", descriptions)

            # Print shift data for each row
            print("\n3. SHIFT DATA BY ROW:")
            for i in range(len(canteen_names)):
                print(f"\nRow {i}:")
                print(f"Canteen: {canteen_names[i]}")
                print(f"Time: {start_times[i]} - {end_times[i]}")
                print(f"Description: {descriptions[i]}")
                
                # Get shifts for this row
            ...
            try:
                ...
                print("\nReceived form data:")
                print(f"Number of canteens: {len(canteen_names)}")
                ...
                print(f"\nExisting timings in database: {existing_timings}")
                ...
                for i in range(len(canteen_names)):
                    ...
                    try:
                        if canteen_name in existing_timings:
                            ...
                            print(f"\nUpdating existing timing ID: {timing_id}")
                            ...
                            update_query = """
                                ...
                                WHERE id = ?
                            """
                            print(f"Update query parameters: {(start_time, end_time, description, timing_id)}")
                            cursor.execute(update_query, (start_time, end_time, description, timing_id))
                            
                            print(f"Deleting existing shifts for timing ID: {timing_id}")
                            cursor.execute("DELETE FROM canteen_timing_shifts WHERE timing_id = ?", (timing_id,))

                        else:
                            print("\nInserting new timing")
                            insert_query = """
                                ...
                                VALUES (?, ?, ?, ?)
                            """
                            print(f"Insert parameters: {(canteen_name, start_time, end_time, description)}")
                        ...
                        # Insert shifts
                        print(f"\nInserting shifts for timing ID {timing_id}:")
                        for shift in selected_shifts:
                            print(f"  Inserting shift: {shift}")
                            cursor.execute("""

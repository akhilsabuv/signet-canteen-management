...
@system_bp.route('/live/assign_canteen_to_shift', methods=['GET', 'POST'])
def assign_canteen_to_shift():
    try:
        ...
        if request.method == 'POST':
            # Get all shift IDs from the form
            shift_ids = request.form.getlist('shift_id')
            
            # First, clear all existing assignments
            cursor.execute("UPDATE shifts SET assigned_canteen_id = NULL")
            
            # Process each shift's canteen assignments
            for shift_id in shift_ids:
                # Get selected canteen IDs for this shift
                canteen_ids = request.form.getlist(f'canteen_id_{shift_id}')
                
                if canteen_ids:
                    # Update the shift with the assigned canteen
                    # Note: Currently supporting one canteen per shift
                    cursor.execute("""
                        UPDATE shifts 
                        SET assigned_canteen_id = ? 
                        WHERE id = ?
                    """, (canteen_ids[0], shift_id))
        ...
        shifts_list = [
            {
                ...
                'assigned_canteen_id': row.assigned_canteen_id
            } for row in shifts
        ]

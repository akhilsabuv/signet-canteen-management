...
def assign_canteen_to_shift():
    ...
    if request.method == 'POST':
        try:
            ...
            # Checkboxes are named as: assignments[<canteen_id>][]
            assignments = {}
            for key in request.form:
                if key.startswith("assignments["):
                    # Extract the canteen id from the key "assignments[<canteen_id>]"
                    canteen_id = key.split("[")[1].split("]")[0]
                    assignments[canteen_id] = request.form.getlist(key)
            
            # Build list of tuples (timing_id, shift_id)
            pairs = []
            for timing_id, shift_list in assignments.items():
                for shift_id in shift_list:
                    pairs.append((timing_id, shift_id))
            
            # Insert pairs into the table.
            conn = get_logger_db_conn()
            cursor = conn.cursor()
            
            # Optionally, clear out old assignments first.
            # Example: DELETE FROM canteen_timing_shifts for the given timing_ids
            timing_ids = list(assignments.keys())
            if timing_ids:
                placeholders = ','.join('?' for _ in timing_ids)
                delete_query = f"DELETE FROM canteen_timing_shifts WHERE timing_id IN ({placeholders})"
                cursor.execute(delete_query, timing_ids)
            
            insert_query = """
                INSERT INTO canteen_timing_shifts (timing_id, shift_name)
                VALUES (?, ?)
            """
            cursor.executemany(insert_query, pairs)
            conn.commit()
            conn.close()
    ...
    # For GET requests, fetch data from the database.
    conn = get_logger_db_conn()
    cursor = conn.cursor()
    try:
        cursor.execute("""
        ...
        cursor.execute("""
            SELECT timing_id, shift_name 
            FROM canteen_timing_shifts
        """)
        selected_assignments = cursor.fetchall()
        
        # Build a dictionary mapping timing_id to a set of assigned shift ids.
        assigned = {}
        for assignment in selected_assignments:
            try:
                # Convert to integers if possible.
                timing_id = int(assignment["timing_id"])
                shift_id = int(assignment["shift_name"])
            except Exception:
                timing_id = assignment["timing_id"]
                shift_id = assignment["shift_name"]
            if timing_id not in assigned:
                assigned[timing_id] = set()
            assigned[timing_id].add(shift_id)
    except Exception as e:
        shifts = []
        canteen_timings = []
        assigned = {}
    ...
    return render_template('assign_canteen_to_shift.html', 
                           shifts=shifts, 

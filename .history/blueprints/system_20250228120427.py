...
def edit_canteen_timings():
    ...
    try:
        # Ensure all required tables exist with proper schema references
        cursor.execute("""
            IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'canteen_timings')
            BEGIN
                CREATE TABLE [signet_log].[dbo].[canteen_timings] (
            ...
            BEGIN
                CREATE TABLE [signet_log].[dbo].[shifts] (
                    id INT IDENTITY(1,1) PRIMARY KEY,
                ...
                INSERT INTO [signet_log].[dbo].[shifts] (shift_name, start_time, end_time)
                VALUES 
            ...
            BEGIN
                CREATE TABLE [signet_log].[dbo].[canteen_timing_shifts] (
                    id INT IDENTITY(1,1) PRIMARY KEY,
                    timing_id INT NOT NULL,
                    shift_name VARCHAR(50) NOT NULL,
                    FOREIGN KEY (timing_id) REFERENCES [signet_log].[dbo].[canteen_timings](id) ON DELETE CASCADE
                )
            END
        """)
        conn.commit()
        ...
        if request.method == 'POST':
            try:
                ...
                cursor.execute("SELECT id, canteen_name FROM [signet_log].[dbo].[canteen_timings]")
                ...
                for i in range(len(canteen_names)):
                    ...
                    print(f"Processing timing {i}: {canteen_name} with shifts: {selected_shifts}")
                    ...
                    if canteen_name in existing_timings:
                        ...
                        cursor.execute("""
                            UPDATE [signet_log].[dbo].[canteen_timings]
                            SET start_time = ?, end_time = ?, description = ?
                            WHERE id = ?
                        """, (start_time, end_time, description, timing_id))

                        cursor.execute("""
                            DELETE FROM [signet_log].[dbo].[canteen_timing_shifts] 
                            WHERE timing_id = ?
                        """, (timing_id,))

                    else:
                        cursor.execute("""
                            INSERT INTO [signet_log].[dbo].[canteen_timings] 
                    ...
                    for shift in selected_shifts:
                        cursor.execute("""
                            INSERT INTO [signet_log].[dbo].[canteen_timing_shifts] 
                            (timing_id, shift_name)
                            VALUES (?, ?)
                        """, (timing_id, shift))
                        print(f"Inserted shift {shift} for timing {timing_id}")
                ...
                for timing_id in timing_ids_to_delete:
                    ...
                    """, (timing_id,))
                    cursor.execute("""
                        DELETE FROM [signet_log].[dbo].[canteen_timings] 
                        WHERE id = ?
                    """, (timing_id,))
        ...
        cursor.execute("""
            ...
            FROM [signet_log].[dbo].[canteen_timings] ct
        ...
        for timing in timings:
            cursor.execute("""
                SELECT shift_name 
                FROM [signet_log].[dbo].[canteen_timing_shifts] 
                WHERE timing_id = ?
            """, (timing.id,))

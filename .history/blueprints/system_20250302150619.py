...
@app.route('/assign_canteen_to_shift', methods=['GET', 'POST'])
def assign_canteen_to_shift():
    ...
    if request.method == 'POST':
        try:
            ...
            # Process the form_data as needed.
            # For example, extract assignments and update the database.
            # (No debug prints are included here.)
            
            flash("Canteen assignments updated successfully!", "success")
            return redirect(url_for('assign_canteen_to_shift'))
        except Exception as e:
            flash(f"Error processing form data: {str(e)}", "error")
            return redirect(url_for('assign_canteen_to_shift'))
    
    # For GET requests, retrieve data from the database.
    conn = get_logger_db_conn()
    cursor = conn.cursor()
    try:
        cursor.execute("""

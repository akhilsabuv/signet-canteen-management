    ...
    #         # Dynamic time window check
            ...
            if start_time <= currentTime <= end_time:
                ...
                if time_difference < timedelta(hours=24):  # Same as timedelta(days=1)
                    ...
                    print("Canteen Timing date", row.TimingID)


                    if latest_start.time() <= shift_start_time <= latest_end.time():

                        savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 1, "Shift start time is within the range.", 1, "test")
                        break
                    else:
                        if(index==total_count):
                            savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 0, "Shift start time is NOT within the range.", 1, "test")
                        else:
                            pass

                else:
                    savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 2, "The user got entered more than 24 hrs ago", 1, "test")

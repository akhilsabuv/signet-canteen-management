...
def edit_canteen_timings():
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                # Start a transaction (all operations below will be atomic)
                ...
                # (Do not commit yet – include deletion in the same transaction)
                ...
                for i in range(len(canteen_names)):
                    ...
                    if timing_id_row is None:
                        raise Exception("Failed to retrieve inserted timing id.")
                    timing_id = timing_id_row[0]
                    print(f"Inserted timing with ID: {timing_id}")

                    # Retrieve the list of shifts for this row.
                    # Note: the checkboxes use a name like "shifts_i[]" so that multiple values are collected.
                    shifts = request.form.getlist(f'shifts_{i}[]')
                    print(f"Processing shifts for row {i}: {shifts}")
        ...
        # GET request: Retrieve existing canteen timings
        cursor.execute("""
            SELECT 
                ct.id,
                ct.canteen_name,

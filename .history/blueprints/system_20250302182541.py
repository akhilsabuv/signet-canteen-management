...
def assign_canteen_to_shift():
    ...
    if request.method == 'POST':
        try:
            # The form names are structured as assignments[<canteen_id>][]
            assignments = {}
            for key in request.form:
                if key.startswith("assignments["):
                    # Extract the canteen id from the key.
                    # The key will be in the form "assignments[<canteen_id>]"
                    canteen_id = key.split("[")[1].split("]")[0]
                    # Get all shift IDs for this canteen.
                    shift_ids = request.form.getlist(key)
                    assignments[canteen_id] = shift_ids

            # Create a list of tuples (canteen_id, shift_id)
            pairs = []
            for canteen_id, shift_list in assignments.items():
                for shift_id in shift_list:
                    pairs.append((canteen_id, shift_id))
            
            # At this point, pairs contains data like:
            # [('1', '3'), ('1', '5'), ('2', '4'), ...]
            # Use these tuples to create your INSERT queries.
            # For example:
            # INSERT INTO your_table (canteen_id, shift_id) VALUES (?, ?)
            #

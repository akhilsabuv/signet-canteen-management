            ...
            if start_time <= currentTime <= end_time:
                ...
                if time_difference < timedelta(hours=24):  # Same as timedelta(days=1)
                    if latest_start.time() <= shift_start_time <= latest_end.time():
                        ...
                        savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 1, "Shift start time is NOT within the range.")
                else:
                    savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 1, "The user got entered more than 24 hrs ago")
                break

                ...
                if time_difference < timedelta(hours=24):  # Same as timedelta(days=1)
                    ...
                    if latest_start.time() <= shift_start_time <= latest_end.time():
                        ...
                        if(coupon_elegible(usrid, event_dt, row.TimingID)):
                            savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 1, "Shift start time is within the range.", row.TimingID, row.canteen_name)
                        else:
                            savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 0, "Shift start time is within the range.", row.TimingID, row.canteen_name)
                        break
                    else:

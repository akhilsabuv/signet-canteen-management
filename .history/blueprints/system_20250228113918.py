...
def get_table_name(table_base_name):
    """Get fully qualified table name using configuration"""
    config = current_app.config
    db_name = config.get('LOGGER_DB_NAME')
    schema = config.get('DB_SCHEMA', 'dbo')  # Default to 'dbo' if not specified
    return f"[{db_name}].[{schema}].[{table_base_name}]"
...
def edit_canteen_timings():
    ...
    canteen_timings_table = get_table_name('canteen_timings')
    shifts_table = get_table_name('shifts')
    timing_shifts_table = get_table_name('canteen_timing_shifts')
    ...
    try:
        ...
        print(f"Error creating shifts table: {str(e)}")
        conn.rollback()
    ...
    if request.method == 'POST':
        try:
            ...
            cursor.execute(f"SELECT id, canteen_name FROM {canteen_timings_table}")
            ...
            for i in range(len(canteen_names)):
                ...
                # Modified shift handling
                selected_shifts = []
                for key in request.form:
                    if key.startswith(f'shifts[{i}]'):
                        shift_value = request.form[key]
                        if shift_value:  # Only add non-empty values
                            selected_shifts.append(shift_value)
                ...
                if canteen_name in existing_timings:
                    ...
                    cursor.execute(f"""
                        UPDATE {canteen_timings_table}
                        SET start_time = ?, end_time = ?, description = ?
                        WHERE id = ?
                    """, (start_time, end_time, description, timing_id))

                    # Clear existing shifts
                    cursor.execute(f"DELETE FROM {timing_shifts_table} WHERE timing_id = ?", (timing_id,))

                else:
                    # Insert new timing
                    cursor.execute(f"""
                        INSERT INTO {canteen_timings_table} (canteen_name, start_time, end_time, description)
                        OUTPUT INSERTED.id
                        VALUES (?, ?, ?, ?)
                ...
                # Insert new shifts
                if selected_shifts:  # Only process if there are shifts selected
                    for shift in selected_shifts:
                        print(f"Inserting shift {shift} for timing {timing_id}")
                        cursor.execute(f"""
                            INSERT INTO {timing_shifts_table} (timing_id, shift_name)
                            VALUES (?, ?)
                        """, (timing_id, shift))
            ...
            print(f"Error during save: {str(e)}")  # Log the error
        ...
        try:
            ...
            cursor.execute(f"""
                ...
                FROM {canteen_timings_table} ct
                ORDER BY ct.start_time
            """)
            timings = cursor.fetchall()
            
            # Debug print to check what data we're getting
            print("Raw Timings Data:", [(t.id, t.canteen_name, t.start_time, t.end_time, t.description) for t in timings])
            ...
            # Create a list to hold timing data with shifts
            timing_data = []
            ...
            for timing in timings:
                cursor.execute(f"""
                    SELECT shift_name 
                    FROM {timing_shifts_table}
                    WHERE timing_id = ?
                """, (timing.id,))
                shifts = [row.shift_name for row in cursor.fetchall()]

                # Create a dictionary for each timing with proper time formatting
                timing_dict = {
                    'id': timing.id,
            ...
            print("Final Timing Data:", timing_data)  # Debug print to verify the data
            ...
            print(f"Error loading page: {str(e)}")  # Log the error
            flash("An error occurred while loading the timings. Please try again.", "error")
            return redirect(url_for('system.edit_canteen_timings'))

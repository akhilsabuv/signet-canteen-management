...
def edit_canteen_timings():
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                for i in range(len(canteen_names)):
                    ...
                    timing_id = timing_ids[i] if i < len(timing_ids) else None
                    selected_shifts = request.form.getlist(f'shifts_{i}')


                    if timing_id and timing_id != 'new':
                        # Update existing timing
                        processed_timing_ids.add(int(timing_id))
                        cursor.execute("""
                            UPDATE canteen_timings 
                            SET canteen_name = ?, start_time = ?, end_time = ?, description = ?
                            WHERE id = ?
                        """, (canteen_name, start_time, end_time, description, timing_id))
                ...
                all_timing_ids = {row.id for row in cursor.fetchall()}
                timing_ids_to_delete = all_timing_ids - processed_timing_ids

                for tid in timing_ids_to_delete:
                    cursor.execute("DELETE FROM canteen_timing_shifts WHERE timing_id = ?", (tid,))
                    cursor.execute("DELETE FROM canteen_timings WHERE id = ?", (tid,))
        ...
        # Get shifts
        cursor.execute("SELECT DISTINCT shift_name, id FROM shifts ORDER BY shift_name")
        allowed_shifts = [{'name': row.shift_name, 'id': row.id} for row in cursor.fetchall()]

...
def savetodb(usrid, event_dt, event_time, latest_entry, shift_start_time, status, description):
    ...
    sql = """
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """
    # params = (usrid, event_dt, event_time, event_timestamp, shift_start_time, status, description)
    print(event_timestamp)

    print(f"usrid type: {type(usrid)}")
    print(f"event_dt type: {type(event_dt)}")
    print(f"event_time type: {type(event_time)}")
    print(f"latest_entry type: {type(event_timestamp)}")
    print(f"shift_start_time type: {type(shift_start_time)}")

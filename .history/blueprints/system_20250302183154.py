...
def assign_canteen_to_shift():
    ...
    if request.method == 'POST':
        try:
            # Process the form data. The checkboxes are named as:
            # assignments[<canteen_id>][] and return a list of shift ids per canteen.
            assignments = {}
            for key in request.form:
                if key.startswith("assignments["):
                    # Extract the canteen id from "assignments[<canteen_id>]"
            ...
            insert_query = """
                ...
                VALUES (?, ?)
            """
            # You may want to clear old assignments before inserting new ones.
            # For example:
            # cursor.execute("DELETE FROM canteen_timing_shifts WHERE timing_id IN (?)", (list(assignments.keys()),))
    ...
    # For GET requests, fetch the shifts, canteen timings, and current assignments.
    conn = get_logger_db_conn()
    cursor = conn.cursor()
    try:
        cursor.execute("""
        ...
        # Retrieve the inserted assignments.
        cursor.execute("""
            SELECT id, timing_id, shift_name 

...
def edit_canteen_timings():
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                for timing in data:
                    ...
                    if timing_id != 'new' and int(timing_id) in existing_ids:
                        ...
                        processed_ids.add(int(timing_id))
                        ...
                        processed_ids.add(timing_id)

                    # Insert shifts
                    for shift in timing['timing_shifts']:
                        cursor.execute("""
                            INSERT INTO canteen_timing_shifts 
                            (timing_id, shift_name)
                            VALUES (?, ?)
                        """, (timing_id, shift['shift_name']))

                # Delete removed timings
                ids_to_delete = existing_ids - processed_ids
                for old_id in ids_to_delete:
                    cursor.execute("DELETE FROM canteen_timing_shifts WHERE timing_id = ?", (old_id,))
                    cursor.execute("DELETE FROM canteen_timings WHERE id = ?", (old_id,))
        ...
        cursor.execute("""
            ...
            ORDER BY shift_name
        """)
        allowed_shifts = [dict(zip(['id', 'name', 'start_time', 'end_time'], 
                                 [row.id, row.shift_name, row.start_time, row.end_time])) 
        ...
        for timing in timings:
            cursor.execute("""
                SELECT cts.id, cts.shift_name, s.*
            ...
            timing_data.append({
                ...
                'shifts': [row.shift_name for row in shifts]
            })

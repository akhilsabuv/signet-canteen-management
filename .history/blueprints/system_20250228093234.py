...
def live_data():
    ...
    try:
        ...
        if active_timings:
            ...
            for timing in active_timings:
                current_timings.append({
                    'canteen': timing.canteen_name,
                    'timing': f"{timing.start_time} - {timing.end_time}"
                })
        else:
            current_timings = [{'canteen': 'No Active Canteen', 'timing': '--:-- - --:--'}]
        ...
        # Get canteen stats with active status
        cursor.execute("""
            SELECT 
                ct.canteen_name,
        ...
        return jsonify({
            'stats': {
                ...
                'current_timings': current_timings  # Changed to send multiple timings
            },

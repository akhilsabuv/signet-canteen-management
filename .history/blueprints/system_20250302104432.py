...
def edit_canteen_timings():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for canteen_name, start_time, end_time, description in zip(canteen_names, start_times, end_times, descriptions, ids):
                if canteen_name and start_time and end_time:
                    cursor.execute(""" 
                    UPDATE canteen_timings 
                    SET start_time = ?, end_time = ?, description = ? 
                    WHERE id = ? 
                    """, (start_time, end_time, description, canteen_name))
            conn.commit()

    ...
    #         # Dynamic time window check
            ...
            if start_time <= currentTime <= end_time:
                ...
                if time_difference < timedelta(hours=24):  # Same as timedelta(days=1)
                    ...
                    print("Event Timing", latest_entry.time())
                    
                    print("Entry Timing", event_dt)
                    ...
                    if latest_start.time() <= shift_start_time <= latest_end.time():
                        savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 1, "Shift start time is within the range.")
                    else:
                        # if(index==total_count):
                        savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 0, "Shift start time is NOT within the range.")
                        # else:
                        #     pass
                else:
                    savetodb(usrid, event_dt, event_dt.time(), latest_entry, shift_start_time, 0, "The user got entered more than 24 hrs ago")
...
###############################################
def savetodb(usrid, event_dt, event_time, latest_entry, shift_start_time, status, description):    # event_timestamp = latest_entry[0]  # Extract the datetime object from the Row

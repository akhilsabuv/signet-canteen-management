...
def edit_canteen_timings():
    ...
    # Add debug print to verify function is called
    print("edit_canteen_timings function called with method:", request.method)
        ...
        try:
            ...
            for key, value in request.form.items():
                if key.startswith("canteen_name_") and not key.startswith("new_"):
                    ...
                    print(f"Processing existing entry: {canteen_name}, {start_time}, {end_time}, {description}")  # Debug print
            ...
            print(f"New entries received: {len(new_names)} items")  # Debug print
            
            for i in range(len(new_names)):
                if new_names[i].strip():  # Only insert if name is not empty
                    print(f"Processing new entry: {new_names[i]}")  # Debug print
                    cursor.execute("""
                        INSERT INTO canteen_timings (canteen_name, start_time, end_time, description)
                        VALUES (?, ?, ?, ?)

    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                ids = request.form.getlist('id[]')  # Assuming you have a hidden input for IDs
                
                # Debugging: Print lengths of each list
                print(f"Lengths - canteen_names: {len(canteen_names)}, start_times: {len(start_times)}, end_times: {len(end_times)}, descriptions: {len(descriptions)}, ids: {len(ids)}")
                
                # Check if all lists have the same length
                if not (len(canteen_names) == len(start_times) == len(end_times) == len(descriptions) == len(ids)):
                    raise ValueError("Form data lists are not of the same length.")
                ...
                for i in range(len(canteen_names)):
                    ...
                    timing_id = ids[i].strip() if ids[i] else None
                    
                    if timing_id:  # Update existing row
                        print(f"Updating row {timing_id}: {canteen_name}, {start_time}, {end_time}, {description}")
                        update_query = """
                            UPDATE canteen_timings
                            SET canteen_name = ?, start_time = ?, end_time = ?, description = ?
                            WHERE id = ?
                        """
                        cursor.execute(update_query, (canteen_name, start_time, end_time, description, timing_id))
                    else:  # Insert new row
                        print(f"Inserting new row: {canteen_name}, {start_time}, {end_time}, {description}")
                        insert_query = """
                ...
                print("Inserted/Updated rows after commit:", inserted_rows)

...
def assign_canteen_to_shift():
    ...
    if request.method == 'POST':
        try:
            ...
            cursor.execute("DELETE FROM canteen_shift_assignments")

            # Process the form data
            for key in request.form:
                if key.startswith("shift_id_"):
                    canteen_id = key.split("_")[2]  # Extract canteen ID from the key
                    shift_id = request.form[key]  # Get the shift ID from the form
                    cursor.execute("""
                        INSERT INTO canteen_shift_assignments (canteen_id, shift_id)
                        VALUES (?, ?)
                    """, (canteen_id, shift_id))

            conn.commit()
            flash("Canteen shift assignments updated successfully.", "success")
        except Exception as e:
            conn.rollback()
            flash(f"Error updating canteen shift assignments: {str(e)}", "error")
        finally:
            conn.close()
        return redirect(url_for('system.assign_canteen_to_shift'))

    # GET request: Retrieve canteens and shifts
    try:
        cursor.execute("SELECT id, canteen_name FROM canteens")
        canteens = cursor.fetchall()

        cursor.execute("SELECT id, shift_name FROM shifts")
        shifts = cursor.fetchall()

        # Retrieve existing assignments for the canteens
        assignments = {}
        cursor.execute("SELECT canteen_id, shift_id FROM canteen_shift_assignments")
        for canteen_id, shift_id in cursor.fetchall():
            if canteen_id not in assignments:
                assignments[canteen_id] = []
            assignments[canteen_id].append(shift_id)

        # Attach assigned shifts to canteens
        for canteen in canteens:
            canteen.assigned_shifts = assignments.get(canteen.id, [])

    except Exception as e:
        flash(f"Error retrieving canteens or shifts: {str(e)}", "error")
        canteens = []
        shifts = []
    finally:
        conn.close()

    return render_template('assign_canteen_to_shift.html', canteens=canteens, shifts=shifts)

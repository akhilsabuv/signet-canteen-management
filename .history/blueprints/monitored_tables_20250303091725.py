...
def canteenandshitchecker():
    try:
        conn = get_logger_db_conn()
        cursor = conn.cursor()

        query_tocheck=''' SELECT ct.id AS TimingID, ct.canteen_name, ct.start_time AS CanteenStartTime, ct.end_time AS CanteenEndTime, s.id AS ShiftID,
